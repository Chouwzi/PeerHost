...
class SessionManager:
  ...
  def load_token(self) -> str | None:
    ...
    with open(TOKEN_FILE, "r") as f:
      token = f.read().strip()
      logger.debug(f"[Session] Đã load token: {token}")
      return token
  ...
  def save_token(self, token: str) -> bool:
    try:
      ...
      with open(TOKEN_FILE, "w") as f:
        f.write(token)
      logger.info(f"[Session] Lưu token thành công | {token[:20]}...")
      return True
    except Exception as e:
      logger.error(f"[Session] Lưu token thất bại | {e}")
      return False
  ...
  def claim_session(self) -> bool:
    ...
    try:
      ...
      logger.info("[Session] Claim Session thành công")
      return True
    except req.ConnectionError as e:
      logger.error(f"[Session] Kết nối đến Server thất bại")
      return False
  ...
  def heartbeat_session(self) -> bool:
    ...
    try:
      ...
      logger.error(f"[Session] Lỗi khi heartbeat: {e}")
      return False
  ...
  def get_session(self) -> dict | None:
    ...
    try:
      ...
      logger.error(f"[Session] Lỗi khi lấy thông tin Session: {e}")
      return None
